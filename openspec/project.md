...
- **转译**: esbuild-wasm(浏览器内 JSX/TS → JS,跑在 Web Worker;前身 my-playground 用的 Babel standalone 已替换,原因见 `docs/frontend-transpile.md`)
...
- 非目标:真鉴权/账号/多人协作(二期用匿名 owner id 隔离,非鉴权)、任意 npm 包(WebContainers 仅远期备选)、多语言/多框架、移动端。
- **后端数据库持久化已于二期纳入**(Postgres/Neon,原为非目标),详见 `docs/backend-design.md` 与 `REQUIREMENTS.md` v0.2。
- 一期范围:单文件、固定依赖、非流式。二期拆两个 change:① 持久化+多轮+历史 ② 真·多文件执行。
